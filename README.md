...
見守りサービスの寿命を決めるのは検知率ではなく**誤報率**です。何でもない日に通知が飛べば家族は通知を切り、切られた瞬間に検知率がいくつであっても価値はゼロになります。検知率は「全部に鳴らせば100%」で作れてしまう一方、その代償である**平常日のコストを公表している見守り製品はほとんどありません**。

そこで、危険判定 `RiskAssessmentService.Evaluate` が**引数だけで完結する純関数**（DBもクロックも触らない）であることを利用して、ラベル付きの1日シナリオを再生する評価ハーネスを置いています。
...
- LLMも外部APIも使わないので、**毎pushでCIが再計算**します（意図分類の評価は課金があるため `MIMAMORI_EVAL=1` のときだけ）。静かな夜に家族を起こすようになった変更は、**それを起こしたプルリクエストでCIが赤くなります**。
- 通知の閾値は `new WatchAlertSettings().Threshold` を読むので、出荷時の既定値を変えれば測定も追随します。
- 結果とシナリオ全52件の一覧、そして**この数字の限界**は [`docs/eval/false-alarm-rate.md`](docs/eval/false-alarm-rate.md)。合成シナリオであること、仕様書と回帰ゲートを兼ねるもので第三者検証ではないことを先に書いてあります。

**この評価は最初の実行で、出荷済みの規則に実際の誤報を1件見つけました。**「活動量が普段より少ない」の判定が、進行中の一日の集計を終わった一日の平均と比べていたため、朝は少なくて当たり前で全世帯が毎朝鳴る状態でした。しかも利用回数が1回以上あることが条件なので、鳴る引き金は**起きて照明を点けたこと**でした。修正は `RiskAssessmentService.LowActivityEarliestHour`（一日がその判断に足るだけ進むまで比較しない。朝の異変は元々「起床が遅い」規則が直接見ている）。再現ケースを n-31 / n-32 として残しています。
