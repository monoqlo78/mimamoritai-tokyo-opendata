...
## 6. 意図分類の精度（100件のラベル付き評価セット）

「AIは言葉に、判断はルールに」という設計が実際に効いているかを、**ラベル付き評価セット100件**で測定しています。

| 項目 | 値 |
| --- | --- |
| 評価件数 | 100 件 |
| 正答 | 97 件（**正答率 97.0%**） |
| うち決定的ロジックだけで確定 | 19 件（LLM 呼び出しなし） |
| モデル呼び出し | 81 件 |
| **危険側の取りこぼし**（緊急・専門家照会を取り逃した件数） | **0 件** |

- 評価セット: `tests/MimamoriTai.Tests/Eval/intent-cases.json`
- 計測結果（自動生成）: [`docs/eval/intent-accuracy.md`](eval/intent-accuracy.md)
- ハーネス: `tests/MimamoriTai.Tests/Eval/IntentEvaluationHarness.cs`

設計上の要点:

- **LLM 単体ではなく、出荷される経路そのものを end-to-end で測っています。** 緊急ワード判定 → 専門家照会 → FAQ → LLM という決定的レイヤのラダーを含めて計測しており、ハーネスは `AssistantOrchestrator.SystemPrompt` の実物を参照します（プロンプトのコピーを測ると、本番と乖離した数字になるため）。
- **総合正答率よりも「危険側の取りこぼし」を優先指標にしています。** 緊急・専門家照会のケースを取り逃すことだけが実害に直結するため、テストは総合正答率をアサートせず、この件数が 0 であることだけをアサートします（総合正答率はモデル更新で動くため、閾値を固定すると意味のない失敗を生みます）。
- **API 費用が発生するため CI では毎push実行せず**、モデル／プロンプト変更時に手動で回して結果をコミットする運用です。データセットの整合性検査（100件ちょうど・ID/文言の重複なし・ラベルの妥当性）は CI で常時走ります。

### 評価セットが実バグを2件検出しました

精度を測るために作った評価セットが、決定的レイヤの取りこぼしを2件見つけました。

1. **「施設の費用の相場はいくら」→「このLINEのやりとりに、お金はかかりません」と誤答。** アプリの料金FAQのキーワード `費用` が広すぎ、介護費用の相談を横取りしていました（**危険側の取りこぼし**）。
2. **「最近は電気の使い方が増えている？」→ アプリの使い方説明を返答。** `使い方` が同様に広すぎました。

対応として `FaqEntry` に**拒否語（`Excludes`）**を導入し、あわせて**専門家照会を FAQ より先に評価する**よう順序を変更しました（どちらも決定的・ゼロコストなので、順序は「同点時にどちらが勝つべきか」の問題であり、安全側が勝つべきという判断です）。境界は `DeterministicLayerBoundaryTests`（15ケース）で固定しています。

この修正により、正答率 96.0%（危険側 1 件）→ **97.0%（危険側 0 件）** になりました。
