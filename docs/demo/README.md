...
**[mimamoritai-demo.mp4](mimamoritai-demo.mp4)** — 4分31秒 / 1920x1080 / 25fps / 16.9MB / 日本語ナレーション + 日本語字幕入り

家電の消費電力と、屋外のオープンデータ（気温・暑さ指数）だけで、離れて暮らすご家族の一日を見守る画面をひととおり操作しています。冒頭で**できること三つ（見守り／ふつうの言葉でのデータ分析／ふつうの言葉での家電操作）**を言い切ってから、**家族が見る画面**と **Microsoft Fabric 上で動く運用コンソール**の両方を見せ、**気温と電気を重ねて見るグラフ**、**ふつうの言葉での質問と、実際に返ってきた AI の回答**、**家電の状態と火気注意**、**「扇風機をつけて」と書くだけで家電が動くところ**、**AI / Microsoft Fabric / LINE の接続状況**まで、実際に動いている画面をそのまま収録したものです。
...
| 0:18 | **Microsoft Fabric 上の運用コンソール**。センサーから家族への通知までのデータの流れ |
| 0:31 | Azure Model Router が実際に答えたモデルと、その平均応答時間 |
| 0:46 | 今日のようす。暑さ指数と、お住まいの地域の気温 |
| 0:57 | 動きはじめた時間 / 最後に動いた時間 / 今日の使用量 |
| 1:08 | 今日の電気を、直近13日間の同時刻平均（点線）と重ねる |
| 1:23 | 起床・就寝の推定を2週間ぶん並べて、生活リズムの変化を見る |
| 1:37 | **気温 × 電気**。気象庁アメダスの最高／最低気温を電力に重ねる |
| 1:54 | 同じ内容を数値で確認。暑い日に電気がのびていれば冷房を使えているサイン |
| 2:07 | 使っているオープンデータ5件（環境省 WBGT / 気象庁アメダス・予報・観測所 / 東京都 熱中症統計） |
| 2:23 | ふつうの言葉で質問（「今日のお母さんの様子は？」）。音声入力にも対応 |
| 2:38 | Azure Model Router が、質問に応じて使うモデルを自動で選ぶ |
| 2:53 | **実際に返ってきた回答**と、根拠になる時刻順のできごと一覧 |
| 3:06 | 家電の状態。火や熱をあつかう機器には注意書きと家族通知 |
| 3:25 | **ふつうの言葉での家電操作**。「扇風機をつけて」と書くと、AI が実行前に必ず聞き返す |
| 3:39 | 「はい」で実行。扇風機が動き、誰がいつ操作したかが記録に残る |
| 3:58 | 接続状況。AI・Microsoft Fabric・LINE の実連携と、選ばれたモデル名 |
| 4:19 | まとめ。カメラを置かず、特別な機器も足さずに |
...
- 終盤の家電操作の場面では「扇風機をつけて」と入力するので、**収録前に扇風機を停止に戻し、チャット履歴を消して**おきます。ついたままだと AI が「すでについています」と返してしまい、実演になりません。
- SRT には解像度情報がなく、libass は既定の `PlayResY=384` を基準に拡大します。1080p では約2.8倍になるので、`FontSize` などはその前提で小さめに指定しています。

`stt_verify.py` は 18 場面すべてを照合します。`AI` や `Microsoft Fabric` は原稿にひらがなで書いてある（そう読ませたいため）ので、文字列としては一致しませんが、**音としては正しく読めている**ことの確認になります。今回の照合結果は、いちばん低い場面でも一致率 0.74、それ以外は 0.78 以上でした。
